...
class ItemQuantityPromotion implements PromotionInterface
{
        ...
        foreach ($cart->getItems() as $item) {
            if ($item->getCartQuantity() >= $this->minimumQuantity) {
                $discount = new Discount(
                    $item->getCartName(),
                    $this->getDicountItemId($item->getCartId()),
                    $item->getCartQuantity() * $item->getUnitPrice() * $this->discountPercentage / 100
                );
    ...
    {
        foreach ($cart->getItems() as $item) {
            if ((int) $item->getCartQuantity() >= (int) $this->minimumQuantity) {
                return true;
            }
        }
    ...
    protected function getDicountItemId(string $id): string
    {
        return \sprintf('%d_discount', $id);
    }
}
